...
async function loadFavorites() {
    try {
        const response = await fetch(CONFIG.getBackendUrl('/api/favorites'), {
            method: 'GET',
            credentials: 'include'
        });
        ...
        const favoritesWithImages = await Promise.all(
            favorites.map(async (favorite) => {
                try {
                    const imgResponse = await fetch(CONFIG.getBackendUrl(CONFIG.BACKEND.API.IMAGINI + '/' + favorite.id));
