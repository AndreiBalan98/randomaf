...
DECLARE
    ...
    v_localitati_arad TEXT[] := ARRAY['Centru', 'Aurel Vlaicu', 'Gradiste', 'Micalaca', 'Gai', 'Bujac', 'Sannicolau Mic', 'Vladimirescu'];
    v_localitati_bacau TEXT[] := ARRAY['Centru', 'Nord', 'Sud', 'Serbanesti', 'Gheraiesti', 'Izvoare', 'Letea', 'Mioritei'];
    v_localitati_baia_mare TEXT[] := ARRAY['Centru', 'Valea Rosie', 'Vasile Alecsandri', 'Sasar', 'Ferneziu', 'Grivitei', 'Gara', 'Recea'];
    v_localitati_bistrita TEXT[] := ARRAY['Centru', 'Unirea', 'Subcetate', 'Viisoara', 'Sigmir', 'Slatinita', 'Ghinda'];
    v_localitati_botosani TEXT[] := ARRAY['Centru', 'Parcul Tineretului', 'Catamarasti', 'Pacea', 'Tudora', 'Curtesti'];
    v_localitati_braila TEXT[] := ARRAY['Centru', 'Viziru', 'Hipodrom', 'Chercea', 'Obor', 'Radu Negru', 'Lacu Dulce'];
    v_localitati_brasov TEXT[] := ARRAY['Centru', 'Tractorul', 'Racadau', 'Bartolomeu', 'Noua', 'Astra', 'Schei', 'Stupini'];
    v_localitati_bucuresti TEXT[] := ARRAY['Sector 1', 'Sector 2', 'Sector 3', 'Sector 4', 'Sector 5', 'Sector 6', 'Baneasa', 'Aviatorilor', 'Cotroceni', 'Drumul Taberei', 'Militari', 'Titan', 'Berceni', 'Colentina'];
    v_localitati_buzau TEXT[] := ARRAY['Centru', 'Micro 14', 'Micro 5', 'Dorobanti', 'Balcescu', 'Simileasca', 'Brosteni'];
    v_localitati_calafat TEXT[] := ARRAY['Centru', 'Basarabi', 'Ciupercenii Vechi', 'Golenti'];
    v_localitati_calarasi TEXT[] := ARRAY['Centru', 'Mircea Voda', 'Oborul Nou', 'Magureni', 'Dumbrava', 'Ostroveni'];
    v_localitati_campina TEXT[] := ARRAY['Centru', 'Slobozia', 'Voila', 'Campinita', 'Turnatorie'];
    v_localitati_campulung TEXT[] := ARRAY['Centru', 'Grui', 'Visoi', 'Valea Romanestilor', 'Schei'];
    v_localitati_cluj TEXT[] := ARRAY['Centru', 'Manastur', 'Gheorgheni', 'Grigorescu', 'Zorilor', 'Buna Ziua', 'Iris', 'Someseni'];
    v_localitati_constanta TEXT[] := ARRAY['Centru', 'Tomis Nord', 'Tomis III', 'Faleza Nord', 'Inel II', 'Palas', 'Coiciu', 'Km 4-5'];
    v_localitati_craiova TEXT[] := ARRAY['Centru', 'Rovine', 'Brazda lui Novac', 'Lapus', 'Valea Rosie', 'Craiovita Noua', 'Bariera Valcii'];
    v_localitati_deva TEXT[] := ARRAY['Centru', 'Micro 15', 'Micro 16', 'Micro 4', 'Micro 5', 'Aurel Vlaicu', 'Grigorescu'];
    v_localitati_drobeta_turnu_severin TEXT[] := ARRAY['Centru', 'Crihala', 'Schela', 'Gura Vaii', 'Dudasu', 'Banovita'];
    v_localitati_focsani TEXT[] := ARRAY['Centru', 'Sud', 'Nord', 'Obor', 'Bahne', 'Gara', 'Mandresti'];
    v_localitati_galati TEXT[] := ARRAY['Centru', 'Mazepa', 'Micro 19', 'Micro 21', 'Micro 40', 'Tiglina', 'Dunarea'];
    ...
    v_localitati_pitesti TEXT[] := ARRAY['Centru', 'Trivale', 'Gavana', 'Prundu', 'Razboieni', 'Eremia Grigorescu'];
    v_localitati_ploiesti TEXT[] := ARRAY['Centru', 'Nord', 'Sud', 'Vest', 'Malul Rosu', 'Bariera Bucuresti', 'Mimiu'];
    v_localitati_ramnicu_valcea TEXT[] := ARRAY['Centru', 'Nord', 'Ostroveni', 'Traian', 'Petrisor', 'Cazanesti'];
    v_localitati_ramnicu_sarat TEXT[] := ARRAY['Centru', 'Anghel Saligny', 'Podgoria', 'Bariera Focsani'];
    v_localitati_reghin TEXT[] := ARRAY['Centru', 'Apalina', 'Iernuteni', 'Dedrad', 'Breaza'];
    v_localitati_resita TEXT[] := ARRAY['Centru', 'Govandari', 'Lunca Barzavei', 'Muncitoresc', 'Dealul Crucii'];
    v_localitati_roman TEXT[] := ARRAY['Centru', 'Favorit', 'Petru Rares', 'Mihai Viteazu', 'Nicolae Balcescu'];
    v_localitati_rosiorii_de_vede TEXT[] := ARRAY['Centru', 'Spitalului', 'Nord', 'Sud', 'Est'];
    v_localitati_satu_mare TEXT[] := ARRAY['Centru', 'Micro 17', 'Micro 16', 'Carpati', 'Soarelui', 'Horea'];
    v_localitati_sibiu TEXT[] := ARRAY['Centru', 'Strand', 'Vasile Aaron', 'Hipodrom', 'Turnisor', 'Terezian', 'Lazaret'];
    v_localitati_sighetu_marmatiei TEXT[] := ARRAY['Centru', 'Valea Cufundoasa', 'Iapa', 'Sugau', 'Lazu Baciului'];
    v_localitati_slatina TEXT[] := ARRAY['Centru', 'Progresul', 'Steaua', 'Clocociov', 'Cireasov'];
    v_localitati_slobozia TEXT[] := ARRAY['Centru', 'Garii Noi', 'Mihai Viteazu', 'Sud', 'Vest'];
    v_localitati_suceava TEXT[] := ARRAY['Centru', 'Burdujeni', 'Obcini', 'Itcani', 'George Enescu', 'Areni'];
    v_localitati_targoviste TEXT[] := ARRAY['Centru', 'Micro 6', 'Micro 9', 'Priseaca', 'Sagricom'];
    v_localitati_targu_jiu TEXT[] := ARRAY['Centru', '9 Mai', 'Debarcader', 'Grivitei', 'Barsesti'];
    v_localitati_targu_mures TEXT[] := ARRAY['Centru', 'Dambul Pietros', 'Unirii', 'Tudor', 'Aleea Carpati', 'Cornisa'];
    v_localitati_targu_neamt TEXT[] := ARRAY['Centru', 'Blebea', 'Condreni', 'Humulesti', 'Ozana'];
    v_localitati_targu_secuiesc TEXT[] := ARRAY['Centru', 'Fabricii', 'Kanta', 'Molnar Janos', 'Turia'];
    v_localitati_timisoara TEXT[] := ARRAY['Centru', 'Soarelui', 'Girocului', 'Circumvalatiunii', 'Lipovei', 'Aradului', 'Mehala', 'Iosefin'];
    v_localitati_turda TEXT[] := ARRAY['Centru', 'Oprisani', 'Micro 3', 'Poiana', 'Turda Noua'];
    ...
    v_localitati_vaslui TEXT[] := ARRAY['Centru', 'Moara Grecilor', 'Gara', 'Rediu', 'Balteni'];
    v_localitati_zalau TEXT[] := ARRAY['Centru', 'Dumbrava Nord', 'Bradet', 'Porolissum', 'Meses'];
    v_localitati_zarnesti TEXT[] := ARRAY['Centru', 'Tohanu Vechi', 'Tohanu Nou', 'Prund', 'Balaceanca'];
